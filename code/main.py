...
def eval():
    ...
    oof_preds = np.round((lgb_oof_pred.argmax(1) + xgb_oof_pred.argmax(1) + cat_oof_pred.argmax(1))/3)
    logger.info(f"lgb and xgb and cat ensemble scores: {macro_f1(y_train, oof_preds)}")
    print(pd.Series(oof_preds).value_counts())
...
def predict():
    ...
    test_pred = np.round((lgb_test_pred.argmax(1) + xgb_test_pred.argmax(1) + cat_test_pred.argmax(1))/3)

    test_feature_df["label"] = test_pred
    # print(test_feature_df["label"].value_counts())
